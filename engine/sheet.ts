...
export function applyShopMarks(
  ...
  const byShop = new Map<number, ShopMark[]>();
  ...
  // Client marks arrive in shop-index order; apply in rule order instead:
  // finish the open shop, complete any fully-filled shops, then at most one partial.
  while (byShop.size > 0) {
    const incomplete = getIncompleteShopIndex(sheet, districtId);
    ...
    sheet.districts[districtId].shops[mark.shopIndex]!.marked[
      mark.symbolIndex
    ] = true;
  }
  return null;
